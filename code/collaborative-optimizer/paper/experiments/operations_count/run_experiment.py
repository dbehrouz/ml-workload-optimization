...
if method == 'optimized':
    ...
    ee = ExecutionEnvironment(DedupedStorageManager(), reuse_type=AllComputeReuse.NAME)
    materializer = StorageAwareMaterializer(storage_budget=mat_budget)
    executor = CollaborativeExecutor(ee, cost_profile=profile, materializer=materializer)
else:
...
for setup, pipeline in setup_and_pipelines:
    ...
    list_of_operation_counts = executor.num_of_executed_operations()
    assert len(list_of_operation_counts) == 1
    operation_counts = list_of_operation_counts[0]
    executor.cleanup()
    i += 1
    if i % 50 == 0:
    ...
    with open(result_file, 'a') as the_file:
        ...
        the_file.write(
            '{},{},{},{},{},{},{},{},{},{}\n'.format(EXPERIMENT_TIMESTAMP.strftime("%H:%M:%S"), e_id,
                                                     ...
                                                     should_warmstart, mat_budget, current_score, operation_counts))
