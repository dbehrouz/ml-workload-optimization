...
For example, the three workloads in the use case of Section \ref{subsec-motivational-example} generate up to 125 GB of artifacts.
...
The first algorithm, which we refer to as \textit{Heuristics-based}, utilizes general metrics, i.e., size, access frequency, and compute times, and storage cost of the vertices, and a machine learning specific metric, i.e., the quality of the machine learning models, to decide what artifacts to materialize.
The second algorithm, called \textit{Storage-aware}, extends the first algorithm and considers the duplication rate among the artifacts.
...
\While {$\mathcal{B}'  > 0$}{
	 $\mathcal{M'} \coloneqq  materialized\_nodes(G_E)$\;
