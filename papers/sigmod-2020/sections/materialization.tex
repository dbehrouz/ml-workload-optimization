...
The utility function $\mathcal{U}(v)$ is combines the potential, recreation cost, and size of a vertex.
To define the utility function, we first define the cost of a vertex as the following:  
\[
...
Now, we define the weighted cost-size ratio as:
\[cost_{ws}(v) = \frac{v.f  \times cost(v)}{v.s}\]
The weighted cost-size ratio has the unit $\tfrac{s}/{Kb}$ and indicates how much time do we spend on computing 1 Kb of an artifact.
In our utility function, we prioritize artifacts with larger $cost_{ws}$, since recomputing them is more costly.
We now define the utility function as the linear combination: 
\[
\mathcal{U}(v) \coloneqq  	
		\begin{cases}
		0, & \text{if }  cost_l(v) \ge cost(v)  \\ 
		\dfrac{\alpha p'(v) + (1-\alpha)cost'_{ws}(v)}, & \text{otherwise}
		\end{cases}
\],
where $p'(v)$ and $cost'_{ws}$ are normalized values of $p(v)$ and $cost_{ws}$ (i.e., the values are between 0 and 1).
...
$0 \leq \alpha \leq 1$ indicates the importance of potential.
...
%TODO $\mathcal{M'} \coloneqq  materialized\_nodes(G)$\; no need for this line
\begin{algorithm}[h]
\KwData  {$G$: experiment graph, $\mathcal{B}$: storage budget}
\KwResult {$\mathcal{M}$: set of vertices to materialize}
