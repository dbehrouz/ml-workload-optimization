...
Here, we first compare the cumulative run-time of running the model-benchmarking scenario using our collaborative optimizer and the OpenML baseline (OML).
Then, we investigate the effect of different values of $\alpha$, the parameter controlling the importance of quality, on run-time.
...
\end{figure}
The parameter $\alpha$ indicates the importance of model quality when deciding to materialize an artifact or not.
...
Therefore, we re-run the experiment of Figures \ref{exp-model-materialization} (a) and (b), but instead of using our heuristics-based or storage-aware materializer, we design a materializer that only materializes the model with the top utility function.
We run the OpenML workloads one by one and compare them with the best model so far.
For every run, we vary the value of $\alpha$ from 0 to 1.
$\alpha$ is 1, the materializer always materializes the best model, since it only considers the quality.
Figure \ref{exp-model-materialization} (c) shows the delta in cumulative run-time between each run with different value of $\alpha$, i.e., how much larger the cumulative run-time is when compared to $\alpha=1$.
Since in this experiment, we re-execute the pipeline with the best model, then if we materialize the best model, we incur a smaller cumulative run-time.
In our collaborative optimizer, the default value of $\alpha$ is 0.5. 
$\alpha =0.5$ provides a good balance between workloads that have the goal of training high-quality models and workloads that are more exploratory in nature.
However, when we have prior knowledge of the nature of the workload in a collaborative environment, then we can set $\alpha$ accordingly, i.e., $\alpha>0.5$ for workloads with the goal of training high-quality models and $\alpha<0.5$ for workloads with exploratory data analysis.
