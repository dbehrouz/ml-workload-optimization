...
\[
WC(G) =  \sum\limits_{v \in V}  (1-v.mat) \times v.f \times in\_edge(v).t
\]
...
\hladd{
Then, for every non-materialized vertex, we compute the utility of the vertex.
Then, we start materializing all the vertices, sorted by their utility, until the storage budget is exhausted.
The utility function ($\mathcal{U}(G,v) $) computes the goodness of an artifact with respect to its recreation cost, how often it is used downstream, and the estimated quality gained from the artifact.
To define the utility function, we must first define the recreation cost function which is the following:  
\[
recreation\_cost(G,v) =\sum\limits_{e \in \bigcup\limits_{v_{0}\in roots} path(G, v_{0}, v)} e.t
\]
which is the total time of executing all the operations from the root vertices to $v$.
We also define $tr$ as the data transfer rate between the experiment graph and the machines running the workloads.
Then, the utility function is defined as: 
\[
\mathcal{U}(G,v) = 	
		\begin{cases}
		0, & \text{if }  \dfrac{v.s}{tr} \ge rc(G,v)  \\ 
		\dfrac{v.f \times p(G, v) \times rc(G,v)}{v.s}, & \text{otherwise}.
		\end{cases}
\]
where $p(G, v)$ and $rc(G,v)$ are the potential and recreation cost of $v$ (for ease of readability we replace $potential(G,v)$ with $p(G,v)$ and $recreation\_cost(G,v)$ with $rc(G,v)$) and $v.f$ and $v.s$ are the frequency and size of the vertex.
$ \frac{v.s}{tr}$ is the amount of time required to transfer the content of vertex $v$ from the experiment graph to the machine which is running the workload.
If transfer time is greater than the recomputation cost of an artifact, we do not materialize the artifact.
Otherwise, we materialize vertices which are more costly to recompute, have larger impacts on the overall quality of the experiment graph, and have higher frequency.}
