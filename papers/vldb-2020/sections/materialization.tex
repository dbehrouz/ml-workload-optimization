...
\begin{algorithm}[h]
\KwData  {$G(V,E):$ experiment graph, $\mathcal{B}:$ storage budget of task}
\KwResult {experiment graph with materialized vertices}
$S \coloneqq 0$ \; %\Comment {current size of the materialized artifacts}
\For  {$ v \leftarrow $ roots(G)}{ %\Comment{materialize all the root nodes}
	\If{ $v.mat= 0$ }{
		 $ v.mat \coloneqq 1 $\;
		 $ S \coloneqq S + v.s $\;
	}
...
\For {$v \leftarrow V$}{
	\If{$v.mat = 0$}{
		 $utility\_value \coloneqq \mathcal{U}(G, v)$\;
		 insert $v$ into $PQ$ sorted by the utility\_value\;
	}
}
\While{$PQ.not\_empty()$}{
$v \coloneqq PQ.pop()$\; % \Comment get vertex with max utility
...
\While {$R > 0$}{
	 $prev\_mats \coloneqq  materialized\_nodes(G)$\;
	 $G \coloneqq $ \textit{Artifact-Materialization}($G, R$)\;
