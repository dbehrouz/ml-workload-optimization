...
We propose a meta-algorithm which iteratively invokes Algorithm \ref{algorithm-materialization} (Artifact-Materialization).
...
If the materialization process cannot find any further vertices to materialize, we return the current graph (Line 6).
We compute the compressed size of the graph artifacts (Line 8), by invoking the compress method of the storage manager which computes the size of graph artifacts after deduplication. 
...
Therefore, we update the size of $v_2$ to indicate the amount of storage it needs to fully materialize.
...
\While {$R > 0$}
	...
	\If  {$prev\_mats = materialized\_nodes(G)$}
		\State return $G$
	\EndIf
	\State $compressed\_size$ = $storage\_manager.compress(G)$
