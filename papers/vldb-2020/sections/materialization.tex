...
\todo[inline]{v.out\_degree will be replaced by number of pipelines which $v$ belongs to, once we properly define $pipeline$ in Section \ref{notations-terms}. Furthermore, it should be the tie breaker when two vertices have similar $potential(v) \times recreatio\_cost(G,v) $ values instead of multiplying it with the potential and recreation cost.}
