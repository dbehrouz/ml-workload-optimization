...
\begin{algorithmic}[1]
\Require  $G(V,E)=$ experiment graph, roots = root artifacts of task, $\mathcal{B}=$ storage budget of task
\Ensure experiment graph with materialized vertices
\State $S= 0$ \Comment {current size of the materialized artifacts}
\For  {$v$ in roots} \Comment{materialize all the root nodes}
	\If{$v.mat= 0$}
		\State $v.mat = 1$
		\State $S = S + v.s$
	\EndIf
...
\todo[inline]{$|pipelines(v)|$ should be the tie breaker when two vertices have similar $potential(v) \times recreatio\_cost(G,v) $ values instead of multiplying it with the potential and recreation cost.}
...
We slightly modify the Artifact-Materialization algorithm, where instead of initializing the current size, $S$, to 0 MB on Line 1 of Algorithm \ref{algorithm-materialization}, we pass the current size as an input to the algorithm. 
...
Initially, we compute the materialized graph by calling the Artifact-Materialization algorithm and computing its compressed storage cost by calling the compress method of the storage manager (Lines 1 and 2 of Algorithm \ref{algorithm-compression-aware-materialization}).
...
\State $S$ = $storage\_manager.compress(G\_mat)$
\While {$S < \mathcal{B}$}
	\State $G\_mat^\prime$  = \textit{Artifact-Materialization}($G\_mat, \mathcal{B}, S$)
	\State $S$ = $storage\_manager.compress(G\_mat^\prime)$
	\If  {$S \geq \mathcal{B}$}
		\State return $G\_mat$
	\Else
