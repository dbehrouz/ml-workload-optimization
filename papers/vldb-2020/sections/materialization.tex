...
\[
potential(G, v) \coloneqq  \max\limits_{m \in M(v)} ( \alpha ^ {\mid path(v,m) \mid} \times quality(m) )
\]
...
\[
EQ(G) \coloneqq   \sum\limits_{v \in V}  v.mat \times potential(v)
\]
...
\[
recreation\_cost(G,v) \coloneqq \sum\limits_{e \in \bigcup\limits_{v_{0}\in roots} path(G, v_{0}, v)} e.t
\]
which is the total execution time of all the operations from the root vertices to $v$.
We also define $transfer(v)$ as the amount of times to transfer vertex $v$ from the experiment graph to the machine running the workload.
The $transfer$ function depends on the size of the vertex and the infrastructure type of the system.
Then, the utility function is defined as: 
\[
\mathcal{U}(G,v) \coloneqq  	
		\begin{cases}
		0, & \text{if }  transfer(v) \ge rc(G,v)  \\ 
