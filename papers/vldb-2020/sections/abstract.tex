...
However, this collaborative platforms suffers from two inefficiencies.
...
As a result, only some of the artifacts such as scripts, and models are stored and users must re-execute the scripts and operations to reconstruct the desired artifact.
...
%TODO Maybe we should rephrease "with high expected rates of future reuse" since we are also looking into quality
Since storing all the artifacts is not feasible, we propose two algorithms for selecting the artifacts with likelihood of future reuse.
