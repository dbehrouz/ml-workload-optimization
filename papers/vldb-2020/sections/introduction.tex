...
% Opening
\hladd{Machine learning is the process of analyzing training datasets to extract features and build machine learning models to solve different tasks, such as labeling images based on image content and detecting fraudulent credit card and bank transactions.}
...
\hladd{Many of these workloads are executed in an interactive approach in notebook environments (e.g., Jupyter notebooks \cite{Kluyver:2016aa}), where users can examine the result of every operation.}
...
Using the \textit{experiment graph}, we automatically extract information to optimize the process of design and execution of future machine learning workloads.
...
\hladd{Reuse and warmstarting can create a faster feedback loop which enables users to try out different data transformation operations, models, and model hyperparameters based on the existing result in shorter time which is a requirement of interactive machine learning workloads.}
...
\item We propose automatic reuse and model warmstarting\hldel{, as well as fast hyperparameter tuning} for new workloads using the experiment graph.
\end{itemize}
