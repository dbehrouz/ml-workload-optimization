...
We use some of the popular scripts in the \textbf{Home Credit Default Risk} competition in Kaggle\footnote{https://www.kaggle.com/c/home-credit-default-risk/}.
...
To evaluate the performance of reuse and warmstarting optimizations, we selected two popular scripts from the Home Credit Kaggle competition.
Script A\footnote{https://www.kaggle.com/willkoehrsen/introduction-to-manual-feature-engineering} and Script B\footnote{https://www.kaggle.com/willkoehrsen/start-here-a-gentle-introduction} both read the input data, perform several steps of preprocessing and feature extractions with many visualization steps and train multiple classification models to using the preprocessed and generated features.
The scripts have been copied and executed more than 2000 times.
...
Figure \ref{exp-reuse-kaggle-same-workload} shows the run time of repeated runs of Scripts A and B.
...
However, after the first run, the experiment graph is populated with the workload graphs of each scripts and the reuse optimization returns the requested data without executing the operations.
This reduces the run time to under 10 seconds for Script A and 60 seconds for Script B.
Most of the run time is spent on returning the requested data and visualization.
However, baseline approach (no experiment graph) does not gain performance improvement from previous runs.
