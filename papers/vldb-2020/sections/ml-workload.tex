...
\textbf{ML Workload Data Types and Operations.}
In our system, we support 3 data types.
...
Each data type results from one of the following 3 types of operations.
...
They allow users to write an end-to-end script which performs data loading, data cleaning, and model training.
We refer to this type of workloads as \textit{long-running workloads}.
...
We refer to this type of workloads as \textit{interactive workloads}.
}
...
Therefore, the local optimizer stops the traversal when it reaches a previously computed vertex.
...
For example, in Figure \ref{system-workflow}, the experiment graph has previously materialized the underlying data (Dataset, Aggregate, or Model inside the vertex) of Vertex 5.
Therefore, during the remote optimization, the underlying data of Vertex 5 is transferred to the workload which results in another subgraph with Vertex 3 and Edge $e_4$ pruned.
...
If the experiment graph is not empty, the remote optimizer tries to optimize the workload using the experiment graph.
