...
In the DAG, edges represent the operations in the workload and vertices represent the artifacts, i.e., raw or preprocessed data (represented by data frame objects) and machine learning models resulting from feature engineering and model training operations.
...
If the artifact is a raw or a preprocessed dataset, then its meta-data includes the name, type, and total size of each column of the data and its underlying data is represented by the dataframe object (i.e., pandas dataframe \cite{mckinney-proc-scipy-2010}). f
If the artifact is a machine learning model, its meta-data includes the name, type, hyperparameters, and the error metric of the model and its underlying data is consist of the model weight.
...
\end{cases}
\]
Intuitively, the hash of root vertices is their unique identifier (location on disk or download URL) and the hash of other vertices is derived by combining the hash of their direct predecessors and edges which connect them to their predecessors.}
...
\hladd{Furthermore, when computing the hash of a combined vertex, we take the combine order into account.
